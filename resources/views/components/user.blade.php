...
<div class="mb-2" x-data="user{{ $user->id }}">
	...
	>
		...
		<div class="flex justify-start gap-3 w-[25%]">
			...
			<div class="text-center min-w-[30px]" title="Количество дейcтвующих сейчас подписок" x-show="!userStat">
				{{ $user->activeSubscriptions->count() }}
			</div>
			<div class="text-center min-w-[50px]" title="Сумма покупок дейcтвующих сейчас подписок" x-show="!userStat">
				{{ $user->activeSubscriptions->sum('price') }} €
			</div>
			<div class="text-center min-w-[30px]" title="Всего куплено подписок за все время" x-show="userStat">
				{{ $user->subscriptions_count }}
			</div>
			<div class="text-center min-w-[50px]" title="Сумма покупок за все время" x-show="userStat">
					...
					<div class="overflow-hidden rounded-xl bg-white/20">
						<div class="p-2">
							...
							@foreach ($user->fragments as $id => $fragment)
								...
								@unless ($audioActive || $videoActive)
									<div class="flex justify-start py-1 transition gap-x-1 hover:bg-slate-100/10">
										<div class="flex items-center min-w-[150px] pl-2">Фрагмент №{{ $id }}</div>
										<div class="flex items-center gap-3 min-w-[180px]">
											<img class="w-[45px] h-[35px]" src="{{ Vite::asset('resources/images/icon4.png') }}" alt="Презентация">
										...
										<div class="grid items-center grid-flow-col gap-1 auto-cols-max min-w-[400px]">
											@if ($fragment['audio'] ?? false)
												...
												>
													...
													<span>{{ $fragment['audio']['ends_at']->diffForHumans() }}</span>
												</div>
											@endif
										</div>
										<div class="flex items-center gap-1 min-w-[400px]">
											@if ($fragment['video'] ?? false)
												...
												>
													...
													<span>{{ $fragment['video']['ends_at']->diffForHumans() }}</span>
												</div>
											@endif
										</div>
									</div>
